...
parser.add_argument('--netmonitor', default="localhost:5001", help='Hostname/IP and port of the netmonitor server')

args = parser.parse_args()
threadpool_size = int(args.poolsize)
netmonitor = args.netmonitor
...
def get_devices():
    ...
    try:
        url="http://"+netmonitor+"/devices"
        print(f"RETREIVING DEVICES FROM :{url}")
        response = requests.get("http://"+netmonitor+"/devices")
...
def update_device(device):
    ...
    try:
        rsp = requests.put("http://"+netmonitor+"/devices", params={"name": device["name"]}, json=device)
    except requests.exceptions.ConnectionError as e:
