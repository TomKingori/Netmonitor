...
class Capture extends Component {
    ...
    fetchPackets() {
        ...
        let requestUrl = process.env.REACT_APP_netmonitor_HOST + '/capture?num_packets=1000';
    ...
    startCapture() {
        ...
        const {ip, protocol, port, time} = this.state;

        // this.setState({isLoading: true});
        let requestUrl = process.env.REACT_APP_netmonitor_HOST + '/capture?num_packets=10';
        if( ip ) { requestUrl += '&ip=' + ip }
        if( protocol ) { requestUrl += '&protocol=' + protocol }
        if( port ) { requestUrl += '&port=' + port }
        if( time ) { requestUrl += '&capture_time=' + time }
            ...
            <Grid container direction="column">
                <Grid container direction="row" style={{paddingTop: '10px'}}>
                    <Grid item style={{width: '15%', paddingLeft: '10px'}}>
                        ...
                        <br /><br />  <br /><br /> */}
                        <form>
                            <Input placeholder="IP" defaultValue={this.state.ip} onChange={event => this.setState({"ip": event.target.value})} />
                            <Input placeholder="Protocol" defaultValue={this.state.protocol} onChange={event => this.setState({"protocol": event.target.value})} />
                            <Input placeholder="Port" defaultValue={this.state.port} onChange={event => this.setState({"port": event.target.value})} />
                            <Input placeholder="Capture time (seconds)" onChange={event => this.setState({"time": event.target.value})} />
                        </form>
                        <br /><br /><br /><br />
                                    ...
                                    return (
                                        <Grid container direction="row">
                                            <Grid item style={{width: '50%', padding: '10px'}}>
                                                <pre style={{color: 'chartreuse', fontSize: 14}}>
                                                    {rowData.packet_json}
                                                </pre>
                                            </Grid>
                                            <Grid item style={{width: '50%', padding: '10px'}}>
