...
def start_receiving():
    ...
    credentials = pika.PlainCredentials("netmonitorUser", "netmonitorPass")
...
def process_work_request(work_type, work_info):
    ...
    if "netmonitor" not in work_info:
        print(f"!!! 'netmonitor' not present in work_info, cannot continue")
        return
    else:
        netmonitor = work_info["netmonitor"]
        print(f"---> work request received, will send results to {netmonitor}")

    if work_type == CAPTURE:
        work_thread = CaptureThread(netmonitor, work_info)
    elif work_type == PORTSCAN:
        work_thread = PortscanThread(netmonitor, work_info)
    elif work_type == TRACEROUTE:
        work_thread = TracerouteThread(netmonitor, work_info)
    elif work_type == SNOOP:
        work_thread = CaptureThread(netmonitor, work_info, snoop=True)
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Remote worker for netmonitor")
    parser.add_argument(
        "-N",
        "--name",
        default="netmonitor-worker",
        help="Name of this worker; must match what is configured on netmonitor server",
    )
    parser.add_argument(
        "-B",
        "--broker",
    ...
    parser.add_argument(
        ...
        "--heartbeat",
        default="30",
        help="Frequency of heartbeats sent to netmonitor server, in seconds",
    )
